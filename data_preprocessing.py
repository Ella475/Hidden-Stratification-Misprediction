...
def import_process_compas2(discretize=False, risk_class=False, inputDir=DATASET_DIR):
    ...
    cols_propb = ["c_charge_degree", "race", "age_cat", "sex", "priors_count", "days_b_screening_arrest",
    ...
    # Warning
    df['length_of_stay'] = (
        (pd.to_datetime(df_raw['c_jail_out']).dt.date - pd.to_datetime(df_raw['c_jail_in']).dt.date).dt.days)
